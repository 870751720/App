...
`DEPLOY_ENABLED` 用于控制 `docker.yml` 的自动部署 job。默认情况下，`main` 分支镜像发布成功且仓库已配置部署必需 Secrets 后，会通过 SSH 部署到服务器；缺少 `SERVER_HOST`、`SERVER_USER`、`SSH_PRIVATE_KEY`、`DATABASE_URL`、`JWT_SECRET`、`MYSQL_PASSWORD` 或 `MYSQL_ROOT_PASSWORD` 时，deploy job 会安全跳过。需要临时关闭自动部署时，将仓库变量 `DEPLOY_ENABLED` 设为 `false`。
