...
`DEPLOY_ENABLED` 用于控制 `docker.yml` 的自动部署 job。默认情况下，`main` 分支镜像发布成功且仓库已配置 `SERVER_HOST`、`SERVER_USER`、`SSH_PRIVATE_KEY` 这些部署 Secrets 后，会通过 SSH 部署到服务器；需要临时关闭自动部署时，将仓库变量 `DEPLOY_ENABLED` 设为 `false`。
