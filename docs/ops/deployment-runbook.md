...
当前 CI 和镜像发布已经跑通。自动 deploy job 默认在 `main` 分支执行，但前提是仓库已配置必需部署 Secrets：`SERVER_HOST`、`SERVER_USER`、`SSH_PRIVATE_KEY`、`DATABASE_URL`、`JWT_SECRET`、`MYSQL_PASSWORD`、`MYSQL_ROOT_PASSWORD`。缺少任一项时 deploy job 会安全跳过；需要临时关闭自动部署时，将仓库变量 `DEPLOY_ENABLED` 设为 `false`。
...
API_HOST=0.0.0.0
API_PORT=3001
AI_STUDENT_EMAIL=user@app.local
AI_DIAGNOSIS_ENDPOINT=
AI_QUESTION_ENDPOINT=
...
$envLines = @(
  ...
  "AI_ACCESS_TOKEN=$($values['AI_ACCESS_TOKEN'])",
  "AI_STUDENT_EMAIL=$($values['AI_STUDENT_EMAIL'])",
